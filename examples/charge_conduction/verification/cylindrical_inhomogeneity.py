r"""Current density across a circular *inhomogeneity* -- hole, soft, hard and perfectly
conducting -- under a uniform far-field electric field: Ohm's law,
:math:`j=\sigmaE` with :math:`E=-\nabla\phi`
...
:math:`j_2=0` by symmetry, so :math:`j_1` is plotted, normalized by the
far-field current density :math:`j_\infty=\sigma_0E_\infty` in the matrix.
...
current density is :math:`\sigma_1E_\mathrm{in}`, uniform. For the perfect conductor
:math:`E_\mathrm{in}=0` while :math:`\sigma_1\to\infty`, and the product has the
finite limit :math:`-\sigma_0E^*` (:math:`2j_\infty/(1-f)` for a circle, with
:math:`f` the area fraction), which is what is drawn. The numeric solve uses
:math:`\sigma_1/\sigma_0=10^3` (float32 noise grows beyond that), so its interior
...
CASES = {
    ...
    "soft": (0.5, r"soft inclusion, $\sigma_1/\sigma_0=1/2$"),
    "hard": (1.5, r"hard inclusion, $\sigma_1/\sigma_0=3/2$"),
...
def _cuts(field, case):
    """``(xi, j1 along x2, j1 along x1)`` of a ``(3,) + grid.shape`` current density, the
...
def plot_inhomogeneity(label):
    ...
    analytic = case.periodic_current_density(FIELD)

    xi, a1, a2 = _cuts(analytic, case)
    _, n1, n2 = _cuts(numeric, case)
    fig, ax = plt.subplots(figsize=(5.0, 4.0), constrained_layout=True)
    ax.set_xlim(-4, 4)
    analytic_numeric_curve(ax, xi, a1, n1, r"$j_1(x_2)$", "C0", downsample=8)
    analytic_numeric_curve(ax, xi, a2, n2, r"$j_1(x_1)$", "C1", downsample=8)
    ax.set_xlabel(r"$x / r_0$")
    ax.set_ylabel(r"$j_1 / j_\infty$")
    ax.set_title(title)
    component_legend(ax)
    save_all(fig, f"charge_conduction.inhomogeneity_{label}")
    plt.close(fig)
