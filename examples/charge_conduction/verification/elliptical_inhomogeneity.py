r"""Interior current density of an *elliptical* inhomogeneity, whose interior field is
exactly uniform (Eshelby), against its shape and against the anisotropy of the
matrix -- as a solid analytic line and numerical markers (Ohm's law,
:math:`j_i=\sigma_{ij}E_j`).
...
* **Aspect ratio** (``aspect_ratio_soft``/``_hard``): an ellipse of semi-axes
  ...
  a electric field at 45 degrees to the axes so both current density components are nonzero,
  against :math:`a/b`.
* **Matrix rotation** (``matrix_rotation_soft``/``_hard``): a circle in a
  matrix with :math:`\sigma_\mathrm{max}/\sigma_\mathrm{min}=4` (principal
  conductivities :math:`2` and :math:`1/2`, geometric mean :math:`1`), the matrix
  rotated about :math:`x_3` by :math:`\varphi` against a fixed electric field along
  :math:`x_1`. The interior current density has period 180 degrees (a rank-2 tensor) and
  gains a transverse component wherever the principal axes are off the field.

The current density is the interior :math:`\sigma_1E_\mathrm{in}`, normalized by
:math:`\sigma_\mathrm{ref}|\bar E|` with :math:`\sigma_\mathrm{ref}` the matrix
electrical conductivity (isotropic) or its geometric-mean principal value (=1).

The analytic curve is
:meth:`crystallite.verification.charge_conduction.ChargeInclusionCase.periodic_interior_current_density`
...
as the inclusion shrinks. Numerics average the solver's current density over the middle of
the inclusion.
...
CIRCLE_RADIUS = 0.1
INTERIOR_RADIUS = 0.5  # elliptical radius bounding the region averaged for the numeric current density
...
def _shape_case(ratio, contrast):
    ...
    return ChargeInclusionCase(Grid(shape=GRID_SHAPE), 1.0, a, b, contrast=contrast)
...
def _analytic_flux(case, field):
    return list(case.periodic_interior_current_density(field)[:2])
...
def plot_aspect_ratio(label):
    ...
    for i, name in enumerate((r"$j_1$", r"$j_2$")):
    ...
    ax.set_xlabel(r"$a / b$")
    ax.set_ylabel(r"$j_\mathrm{in} / (\sigma_0 |\bar{E}|)$")
    ax.set_title(rf"elliptical inclusion, $\sigma_1/\sigma_0={contrast:g}$: interior current density vs. aspect ratio")
    component_legend(ax)
    save_all(fig, f"charge_conduction.elliptical_inhomogeneity_aspect_ratio_{label}")
    plt.close(fig)
...
def plot_matrix_rotation(label):
    ...
    ax.set_ylabel(r"$j_\mathrm{in} / (\sigma_\mathrm{ref} |\bar{E}|)$")
    ax.set_title(rf"circle, $\sigma_1/\sigma_0={contrast:g}$, $\sigma_\mathrm{{max}}/\sigma_\mathrm{{min}}=4$: interior current density vs. matrix rotation")
    component_legend(ax)
    save_all(fig, f"charge_conduction.elliptical_inhomogeneity_matrix_rotation_{label}")
    plt.close(fig)
