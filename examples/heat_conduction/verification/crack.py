r"""Heat flux ahead of the tip of an insulating crack, under a mean thermal driving force
across it (Fourier's law) -- flow forced around the crack, with the tip singularity
:math:`q_1/q_\infty\to x_2/\sqrt{x_2^2-a^2}` (isolated).
...
(:func:`crystallite.verification.heat_conduction.elliptical_perturbation`) of the
...
def plot_crack():
    ...
    analytic = case.periodic_heat_flux(FIELD, n_modes=N_MODES)
    ...
    component_legend(ax)
    save_all(fig, "heat_conduction.crack")
    plt.close(fig)
