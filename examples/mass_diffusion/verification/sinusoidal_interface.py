...
def plot_amplitude_decay():
    ...
    ax.plot(times, linear_amplitude, color="C0", label="analytical (linear)")
    ax.plot(
        times[::150], nonlinear_amplitude[::150], "o", color="C1", markersize=4,
        label="numerical (nonlinear)",
    )
...
def plot_weight_sensitivity():
    ...
    ax.plot(times, _linear_amplitude(times), color="k", label="analytical")
    for alpha in (0.5, 1.0, 2.0, 4.0):
        t, amplitude = _run_nonlinear(
            alpha=alpha, time_step=weight_time_step, steps=weight_steps
        )
        ax.plot(t, amplitude, "o-", markersize=4, label=rf"$\alpha = {alpha:g}$")
    ax.set_xlabel(r"time $t$")
    ax.set_ylabel(r"amplitude")
    ax.legend()
...
def plot_step_sensitivity():
    ...
    ax.loglog(time_steps, errors, "o-", color="C0", label="MassDiffusion")
    ax.loglog(
        time_steps, errors[-1] * (time_steps / time_steps[-1]),
        "--", color="0.5", label=r"$O(\Delta t)$",
    )
    ax.set_xlabel(r"time step $\Delta t$")
    ax.set_ylabel(r"final-amplitude error vs. fine-$\Delta t$ reference")
    ax.legend()
...
def plot_volume_and_surface_mechanisms():
    ...
    for degeneracy, name in [(False, "volume"), (True, "surface")]:
        ...
        fig, ax = plt.subplots(figsize=(4.5, 3.5), constrained_layout=True)
        ax.plot(times, linear, color="C0", label="analytical (linear)")
        ax.plot(
            times[::150], amplitude[::150], "o", color="C1", markersize=4,
            label="numerical (nonlinear)",
        )
        ax.set_xlabel(r"time $t$")
        ax.set_ylabel(r"amplitude")
